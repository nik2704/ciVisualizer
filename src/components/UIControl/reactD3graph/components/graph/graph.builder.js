...
function buildLinkProps(link, nodes, links, config, linkCallbacks, highlightedNode, highlightedLink, transform) {
  ...
  // ------------------ CUSTOMIZATION THE NEXT BLOCK IS A WORKARAOUND FOR ARRAOWS AND IS ABSENT IN ORIGINAL

  if (!isNaN(x1) && !isNaN(x2) && !isNaN(y1) && !isNaN(y2)) {
    let proectX = Math.abs(x2 - x1);
    let proectY = Math.abs(y2 - y1);
    let lngOfline = Math.sqrt(Math.pow(proectX, 2) + Math.pow(proectY, 2));

    let radius = 20 * transform;

    let delta = lngOfline / radius;
    let deltaX = proectX / delta;
    let deltaY = proectY / delta;
    ...
    if (!isNaN(tmpx1) && !isNaN(tmpx2) && !isNaN(tmpy1) && !isNaN(tmpy2)) {
      x1 = tmpx1;
      x2 = tmpx2;
      y1 = tmpy1;
      y2 = tmpy2;
  ...
  const t = transform;    // CUSTOMIZATION. Original was const t = 1 / transform;
  ...
  return {
    ...
    color: link.color,                                //CUSTOMIZATION
  };
}
...
function buildNodeProps(node, config, nodeCallbacks = {}, highlightedNode, highlightedLink, transform) {
  ...
  // const t = 1 / transform;
  const t = transform;        // CUSTOMIZATION. The original line is const t = 1 / transform;
  ...
  const dx = finalFontSize * t + offset / 100 + 5; // CUSTOMIZATION ORIGINAL WAS finalFontSize * t + offset / 100 + 1.5
